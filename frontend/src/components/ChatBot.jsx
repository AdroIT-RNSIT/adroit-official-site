        ...
        <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end">
            ...
            {!isOpen && (
                <button
                    ...
                    className="group flex items-center gap-2 px-4 py-3 bg-sky-600 rounded-full shadow-lg hover:shadow-sky-600/50 hover:scale-105 transition-all duration-300"
                >
                            ...
                            >
                                <div
                                    className={`max-w-[85%] rounded-2xl px-4 py-2.5 text-sm leading-relaxed ${msg.role === "user"
                                        ? "bg-gradient-to-br from-sky-600 to-sky-700 text-slate-900 rounded-tr-sm"
                                    ...
                                    <ReactMarkdown
                                        ...
                                        components={{
                                            ...
                                            code: ({ node, inline, className, children, ...props }) => {
                                                return inline ? (
                                                    <code className="bg-white/20 rounded px-1 py-0.5 text-xs font-mono text-sky-100" {...props}>{children}</code>
                                                ) : (
                                                    <code className="block bg-white/20 rounded p-2 text-xs font-mono text-sky-100 overflow-x-auto my-2" {...props}>{children}</code>
                                                );
                                            }
                                        }}
                                    >
                        ...
                        {isLoading && (
                            <div className="flex justify-start">
                                <div className="bg-slate-900/5 rounded-2xl px-4 py-3 rounded-tl-sm flex gap-1 items-center">
                                    ...
                                    <span className="w-1.5 h-1.5 bg-sky-500 rounded-full animate-bounce [animation-delay:0.1s]"></span>
                    ...
                    <form onSubmit={handleSubmit} className="p-3 bg-slate-900/5 border-t border-slate-900/10">
                        <div className="relative flex items-center">
                            ...
                            <button
                                ...
                                className="absolute right-2 p-1.5 bg-sky-600 rounded-lg text-slate-900 disabled:opacity-50 disabled:cursor-not-allowed hover:opacity-90 transition-opacity"
                            >
