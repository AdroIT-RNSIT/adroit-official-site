...
export default function Resources() {
  ...
  const filteredResources =
    domain && domain !== "all"
      ? resources.filter((r) => r.domain === domain)
      : resources;
        ...
        <div className="bg-black/40 backdrop-blur-xl border border-white/10 rounded-2xl p-5 mb-10">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-3">
            ...
            <div className="lg:col-span-2">
              <button
                onClick={() =>
                  setFilters({ type: "", difficulty: "", search: "" })
                }
            ...
            <p className="text-sm text-gray-500">
              ...
              {domain && domain !== "all" && (
                <>
                  ...
                  in{" "}
                  <span
                    className={DOMAINS.find((d) => d.id === domain)?.textColor}
                  >
...
function ResourceCard({ resource, isAdmin, onDelete }) {
  ...
  const difficultyConfig = resource.difficulty
    ? DIFFICULTY_CONFIG[resource.difficulty]
    : null;
      ...
      <div className="relative z-10">
        ...
        <div className="flex items-start justify-between mb-3">
          <div className="flex items-center gap-2">
            <div
              className={`w-10 h-10 rounded-lg bg-gradient-to-br ${typeConfig.color} flex items-center justify-center text-xl`}
            >
              {typeConfig.icon}
            </div>
            <div>
              <span
                className={`inline-block px-2 py-0.5 text-xs font-medium rounded-full bg-gradient-to-r ${typeConfig.color} bg-opacity-20 text-white`}
              >
                {typeConfig.label}
              </span>
              {difficultyConfig && (
                <span
                  className={`ml-1.5 inline-block px-2 py-0.5 text-xs font-medium rounded-full bg-gradient-to-r ${difficultyConfig.color} bg-opacity-20 ${difficultyConfig.textColor}`}
                >
        ...
        <div className="flex items-center justify-between mb-3">
          <span
            className={`inline-flex items-center gap-1 px-2 py-0.5 text-xs rounded-full bg-gradient-to-r ${domain.color} bg-opacity-20 text-white`}
          >
        ...
        {resource.tags && resource.tags.length > 0 && (
          <div className="flex flex-wrap gap-1.5 mb-3">
            {resource.tags.slice(0, 2).map((tag) => (
              <span
                key={tag}
                className="px-1.5 py-0.5 text-[10px] bg-white/5 rounded text-gray-400"
              >
                #{tag}
              </span>
            ))}
